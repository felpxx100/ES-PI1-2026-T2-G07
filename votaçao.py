...
def realizar_voto(conexao, cursor):
    ...
    if len(resultado_candidato) > 0:
        candidato = resultado_candidato[0]
        print(f"\nVocê está a votar em: {candidato[0]} ({candidato[1]})")
        id_candidato = candidato[2]
    else:
        print("\nCandidato não encontrado. Você está a votar NULO/BRANCO.")
        cursor.execute("SELECT id_candidatos FROM Candidatos WHERE digito_candidatos = 99")
        id_candidato = cursor.fetchall()[0][0]
    ...
    if confirma == "SIM":
        ...
        registrar_log("SUCESSO: Voto realizado com sucesso")
        print(f"\n[Voto Confirmado] Guarde o seu protocolo: {protocolo}")
    else:
        print("\nVoto cancelado pelo eleitor.")
...
def encerrar_votacao(conexao, cursor):
    ...
    # Exige autenticação de mesário para fechar a urna
    if validar_credenciais(cursor, titulo, cpf_4, chave, "mesario") == False:
        print("\n[Erro] Falha na validação do mesário. Acesso negado.")
    ...
    print("\nA processar encerramento da urna e a contabilizar os votos...")
    
    # Faz a contagem final dos votos juntando as tabelas Candidatos e Votos
    cursor.execute("""
        SELECT c.nome_candidato, COUNT(v.protocolo) as total_votos
        FROM Candidatos c
        LEFT JOIN Votos v ON c.id_candidatos = v.id_candidatos
        GROUP BY c.id_candidatos, c.nome_candidato
        ORDER BY total_votos DESC
    """)
    resultados = cursor.fetchall()
    
    print("\n--- BOLETIM FINAL DE URNA ---")
    for linha in resultados:
        print(f"Candidato: {linha[0]} | Total de Votos: {linha[1]}")
